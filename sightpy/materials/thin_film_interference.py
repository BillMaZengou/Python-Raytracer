...
class ThinFilmInterference(Material):
    def __init__(self, thickness, noise=0., **kwargs):
        ...
        self.thickness_noise = load_image("sightpy/textures/noise.png")[:, :, 0]
    ...
    def get_color(self, scene, ray, hit):
        ...
        # Ambient
        color = rgb(0., 0., 0.)
        V = -ray.dir                           # direction to ray origin
        ...
        if ray.depth < hit.surface.max_ray_depth:
            ...
            if self.noise_factor != 0.:
                thickness += self.noise_factor * (self.thickness_noise[
                    -(v * self.thickness_noise.shape[0] * 0.5).astype(int) % self.thickness_noise.shape[0], 
                    (u * self.thickness_noise.shape[1] * 0.5).astype(int) % self.thickness_noise.shape[1]  
                ].T - 0.5)
                Fim = self.thin_film_interference_reflectance[
                    (costhetai * self.thin_film_interference_reflectance.shape[0]).astype(int),

...
class Cuboid(Primitive): 
    def __init__(self, center, material, width, height, length, max_ray_depth=5, shadow=True):
        ...
        self.height = height
        self.length = length
        self.bounded_sphere_radius = np.sqrt(self.width**2 + self.height**2 + self.length**2) / 2.
    ...
    def get_uv(self, hit):
        ...
        u = np.select(
            ...
            [
                ...
                ((-self.ax_l).dot(M_C) / self.width * 2 * 0.985 + 1) / 2,    
                ((-self.ax_w).dot(M_C) / self.width * 2 * 0.985 + 1) / 2 + 3,     
                (self.ax_w.dot(M_C) / self.width * 2 * 0.985 + 1) / 2 + 1
            ]
        )
        v = np.select(
